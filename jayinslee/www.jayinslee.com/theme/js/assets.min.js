...
/* trilogy-survey-1.4.0.min.js */
window.Survey={$contactForm:null,$surveyForm:null,$upsell:null,$thanks:null,scene:null,$scenes:[],$fields:[],speed:"fast",logLevel:0,callbacks:{},version:"1.4.0",attrs:{surveyProgress:undefined,surveySteps:undefined,SuccessUrl:undefined}},Survey.init=function(){Survey.$scenes=$(".survey-step").hide().not(".upsell").not(".thanks"),Survey.scene="contact-form",Survey.$contactForm=$("#contact-form"),Survey.$surveyForm=$("#survey-form"),Survey.$upsell=$(".survey-step.upsell"),""!==$(".thanks").find("h2").text()&&(Survey.$thanks=$(".survey-step.thanks")),window.SurveyFields!==undefined&&$.each(window.SurveyFields,function(e,r){Survey.registerField(r.selector,r.rules)}),Survey.updateAttr("surveyProgress",1),Survey.updateAttr("surveySteps",Survey.$scenes.length),Survey.updateAttr("SuccessUrl",Survey.$surveyForm.find("input[name='SuccessUrl']").val()),Survey.$s().fadeToggle(Survey.speed)},Survey.addRules=function(){$.each(Survey.$fields,function(e,r){r.scene===Survey.scene&&(r.el.rules("add",r.rules),Survey.logger("Added rule for "+r.el.attr("id")))})},Survey.back=function(){Survey.scene===Survey.$upsell.attr("data-scene")?Survey.$upsell.fadeToggle(Survey.speed,function(){Survey.$s().fadeToggle(Survey.speed,function(){Survey.scene=$(".survey-step:visible:first").attr("data-scene")})}):Survey.$s().fadeToggle(Survey.speed,function(){Survey.removeRules(),Survey.updateAttr("surveyProgress",Survey.attrs.surveyProgress-1),Survey.$s().fadeToggle(Survey.speed,function(){Survey.scene=$(".survey-step:visible:first").attr("data-scene")})}),window.scrollTo(0,0)},Survey.hasSuccessUrl=function(){return"undefined"!=typeof Survey.attrs.SuccessUrl},Survey.hasUpsell=function(){return Survey.$upsell.length>0},Survey.logger=function(e){switch(Survey.logLevel){case 0:break;case 1:console.log(e)}},Survey.next=function(){switch(Survey.logger(Survey.attrs.surveyProgress),Survey.attrs.surveyProgress){case 1:1===Survey.attrs.surveySteps?Survey.submit():0===Survey.$contactForm.length?Survey.nextScene():Survey.$contactForm.submit();break;case Survey.attrs.surveySteps:Survey.hasUpsell()&&Survey.$upsell.is(":hidden")?(Survey.logger("Going to upsell scene"),Survey.upsell()):Survey.submit();break;default:Survey.nextScene()}},Survey.submit=function(){Survey.logger("Submitting survey form"),Survey.$surveyForm.submit(),Survey.hasSuccessUrl()&&window.setTimeout(function(){window.location=Survey.attrs.SuccessUrl},2e3)},Survey.nextScene=function(e){valid=!1===e||Survey.$surveyForm.valid(),valid&&Survey.$s().fadeToggle(Survey.speed,function(){Survey.updateAttr("surveyProgress",Survey.attrs.surveyProgress+1),Survey.$s().fadeToggle(Survey.speed,function(){Survey.scene=$(".survey-step:visible:first").attr("data-scene"),Survey.addRules()})}),window.scrollTo(0,0)},Survey.removeRules=function(){$.each(Survey.$fields,function(e,r){r.scene===Survey.scene&&(r.el.rules("remove"),Survey.logger("Removed rule for "+r.el.attr("id")))})},Survey.registerField=function(e,r){Survey.$fields.push({el:$(e),scene:$(e).parents(".survey-step").attr("data-scene"),rules:r})},Survey.runCallback=function(e){Survey.callbacks.hasOwnProperty(e)&&Survey.callbacks[e]()},Survey.updateAttr=function(e,r){if(Survey.logger("Updating "+e+" to "+r),$el=$("[data-attr='"+e+"']"),Survey.attrs[e]=r,$el.length>0)switch($el.get(0).tagName){case"INPUT":$el.val(r);break;default:$el.html(r)}},Survey.upsell=function(){Survey.logger("Upsell"),Survey.updateAttr("SuccessUrl",Survey.$upsell.find(".options input:checked").val()),Survey.$s().fadeOut(Survey.speed,function(){Survey.$upsell.show(),Survey.scene=Survey.$upsell.attr("data-scene")})},Survey.$s=function(){return $(Survey.$scenes[Survey.attrs.surveyProgress-1])},$(document).ready(function(){$("#survey-form").length>0&&(Survey.init(),$("body").on("click","button.previous",function(e){e.preventDefault(),Survey.back()}),$("body").on("click","button.next, button[type='submit']",function(e){e.preventDefault(),Survey.$surveyForm.valid()&&(Survey.runCallback("next"),Survey.next())}),$("body").on("click","[data-scene='donate'] input[type='radio']",function(){Survey.updateAttr("SuccessUrl",$(this).val())}))});
